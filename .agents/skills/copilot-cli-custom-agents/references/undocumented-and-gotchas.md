...
| `model` accepts display names | Changelog: "Custom agent model field now accepts display names and vendor suffixes from VS Code (e.g., `Claude Sonnet 4.5`, `GPT-5.4 (copilot)`)" — still a single string, just a wider accepted format. |
