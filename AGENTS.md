...
- `apm pack` is the only generator you must run and commit by hand. The ARD catalog and sitemap workflows regenerate their outputs on `main` and same-repo PR branches. The plugin listing workflow keeps PRs current; after a version bump reaches `main`, the plugin release workflow creates the exact version tag first, then regenerates and commits `index.html` and `sitemap.xml` so each card uses the annotated tag's release date. Fork PRs must run the matching generator locally because CI cannot push to forks.
...
- Per-plugin GitHub Releases are automated: `.github/workflows/plugin-releases.yml` runs `npm run release:plugins` on every push to `main` that touches `apm.yml`. For each package version without a matching tag, it creates and pushes the annotated tag, creates the GitHub Release, then regenerates `index.html` from those tags. Bump the version in root `apm.yml` and `plugins/<name>/apm.yml`; never use a source commit date as a release date.
...
- Each package's `tags:` list in root `apm.yml` (mirrored by that package's own `plugins/<name>/apm.yml` `keywords:`) is capped at 12 entries — `npm run generate:plugins` fails loudly (`MAX_TAGS` check in [scripts/generate-plugins-html.mjs](scripts/generate-plugins-html.mjs)) if a package exceeds it. Keep only genuinely distinguishing search terms; drop synonyms and redundant sub-tags (e.g. don't tag both a skill name and a near-duplicate generic word for it) before adding a new one when a package is already near the cap.
