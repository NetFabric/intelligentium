...
description: "Deterministic guardrails that make .NET/C# repos safe for AI code agents: .editorconfig + dotnet format, TreatWarningsAsErrors, custom Roslyn analyzer projects for architecture rules, full test suites as the behavioral contract, Aspire as the agent-native orchestration/observability layer, and Copilot/Claude Code hooks as the governance layer. USE FOR: writing AGENTS.md/CLAUDE.md rules for a .NET repo used by code agents; defining what 'done' means for agent-generated .NET changes; wiring a Roslyn analyzer project via ProjectReference OutputItemType=Analyzer; enforcing dotnet format exit code 0 and TreatWarningsAsErrors; using Aspire telemetry/traces so agents reason about running systems, not just source; writing hooks that validate or block agent actions. DO NOT USE FOR: general AGENTS.md authoring for non-.NET repos (agents-md); C# style rules (csharp-best-practices); non-agent solution scaffolding (dotnet-solution-setup); BenchmarkDotNet usage (dotnet-benchmarking)."
---
...
Set once in `Directory.Build.props` at the repo root so every project inherits it — see [dotnet-solution-setup](../dotnet-solution-setup/SKILL.md) for the full shared-properties list.
